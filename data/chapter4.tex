...
% 在分支预测中分支历史管理也是一个重要的功能。在第一版的设计中，参考了BOOM，将每个32Byte的取指block视为整体，每个block在全局分支历史中占用1bit，无论其中有多少条指令。而在第二版架构中，为了令全局分支历史更加准确，改为了每一条分支指令在全局历史中占用1bit，这样的话全局分支历史的管理策略就需要做相应的改动。

% 在第二版的架构中除了保存完整的分支历史以外，还保存了折叠的分支历史，不同于第一版，每次需要使用折叠历史时都是用完整的分支历史计算，这次的这叠分支历史是直接按照折叠的状态来维护的。

% 全局分支历史用一个循环队列和指针来维护

% 历史会顺着流水级往下传，每个流水级预测同一个block时拿到的都是相同的分支历史。当出现冲刷流水线时，就把分支历史恢复成它预测的结果。然后有一个优先选择器，每次都更新s0_ghist
